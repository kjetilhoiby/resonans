/**
 * Applies migration 0033_canonical_workouts.
 */
...
try {
  const migrationSql = readFileSync(resolve(__dirname, '..', 'drizzle', '0033_canonical_workouts.sql'), 'utf8');
  for (const stmt of migrationSql.split('--> statement-breakpoint').map((s) => s.trim()).filter(Boolean)) {
    await sql.unsafe(stmt);
  }
  console.log('✅ Migration 0033 applied: canonical_workouts + workout_daily_aggregates');
} catch (err) {
