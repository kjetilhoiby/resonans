...
export const POST: RequestHandler = async ({ locals, request }) => {
	...
	const itemsList = candidates
		.map((i, idx) => {
			const ctx = i.checklistContext ? ` [${i.checklistContext}]` : '';
			return `${idx + 1}. id=${i.id}${ctx}: ${i.text}`;
		})
		.join('\n');

	const systemPrompt = `Du hjelper brukeren sortere oppgaver. For hver oppgave skal du foreslå:
...
${itemsList}`;
	...
	try {
		...
		console.error('[checklist-items/triage-suggest] failed', err);
