...
export const POST: RequestHandler = async ({ locals, url }) => {
	try {
		...
		const result = await syncDropboxWorkoutsForUser(locals.userId, { fullRescan });

		return json({
			success: true,
			fullRescan,
			...result,
			message: `Importerte ${result.imported} fil(er), hoppet over ${result.skipped}, feilet ${result.failed}.`
		});
	} catch (error) {
