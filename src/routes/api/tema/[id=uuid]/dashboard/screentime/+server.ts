...
export const GET: RequestHandler = async ({ params, locals }) => {
	...
	if (resolveThemeDashboardKind(theme.name) !== 'screentime') {
		return json({ error: 'Temaet har ikke skjermtidsdashboard.' }, { status: 400 });
	}

	const dashboard = await loadScreenTimeDashboardData(locals.userId);
