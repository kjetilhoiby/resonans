...
export const GET: RequestHandler = async ({ params, locals }) => {
	...
	if (resolveThemeDashboardKind(theme.name) !== 'weight') {
		return json({ error: 'Temaet har ikke vektdashboard.' }, { status: 400 });
	}

	return json(await loadWeightDashboardData(locals.userId));
};
