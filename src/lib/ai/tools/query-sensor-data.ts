...
- 'raw_events': Get individual measurements (detailed data)
...
The tool returns actual data from Withings sensors that the user can trust.`,
	...
	parameters: z.object({
		...
		queryType: z.enum(['latest', 'period_summary', 'trend', 'raw_events']).describe(
			'latest: Get most recent metrics. period_summary: Get aggregates for a period. trend: Compare multiple periods. raw_events: Get individual measurements.'
		),
		...
		metric: z.enum(['weight', 'steps', 'sleep', 'intense_minutes', 'all']).optional().describe('Which metric to focus on'),
	...
	execute: async (args: {
		...
		periodKey?: string;
		metric?: 'weight' | 'steps' | 'sleep' | 'intense_minutes' | 'all';
		limit?: number;
		startDate?: string;
		endDate?: string;
	}) => {
		...
		try {
			...
			if (queryType === 'raw_events') {
				...
				// Filter by event type if metric specified
				if (metric && metric !== 'all') {
					if (metric === 'steps' || metric === 'intense_minutes') {
						// Only activity events have steps/intense minutes
						conditions.push(eq(sensorEvents.eventType, 'activity'));
					} else if (metric === 'weight' || metric === 'sleep') {
						// Both weight and sleep are in measurement events
						conditions.push(eq(sensorEvents.eventType, 'measurement'));
					}
				}
				...
				const events = await db.query.sensorEvents.findMany({
					...
					limit: limit || 100 // Increased from 50
				});

				console.log(`   Found ${events.length} raw events (metric: ${metric || 'all'}, conditions: ${conditions.length})`);
				...
				// Further filter by specific metric if needed
				...
				if (metric && metric !== 'all') {
					filteredEvents = events.filter(e => {
						...
						if (metric === 'weight') {
							...
							return data?.sleepDuration !== undefined;
						}
						return true; // For steps/intense_minutes, already filtered by eventType
					});
				}

				console.log(`   After metric filter: ${filteredEvents.length} events`);

				if (filteredEvents.length === 0) {
					return {
						success: false,
						message: `No ${metric} data found in the retrieved events`
					};
				}
				...
				return {
					...
					message: `Found ${filteredEvents.length} raw ${metric || 'sensor'} events`
				};
			}
