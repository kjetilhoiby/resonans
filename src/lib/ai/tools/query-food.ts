...
import { meals, mealPlans, pantryItems } from '$lib/db/schema';
import { eq, and, gte, lte, desc, asc } from 'drizzle-orm';

export const queryFoodTool = {
	name: 'query_food',
	description: `Read user's food data: meals, weekly meal plans, and pantry/freezer contents.

Query types:
- 'meals': List the user's saved meals (limit 50)
...
- 'expiring_soon': List pantry items expiring within N days (default 7)`,
	...
	parameters: z.object({
		...
		queryType: z.enum(['meals', 'meal_plan', 'pantry', 'expiring_soon']),
	...
	execute: async (args: {
		userId: string;
		queryType: 'meals' | 'meal_plan' | 'pantry' | 'expiring_soon';
		weekContext?: string;
		...
		switch (args.queryType) {
			case 'meals': {
				const rows = await db
					.select()
					.from(meals)
					.where(eq(meals.userId, args.userId))
