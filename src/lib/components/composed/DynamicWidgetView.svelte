<!--
  DynamicWidgetView — ren presentasjon av en hjemskjerm-widget.
  Props-drevet (data/loading/error) — datahenting bor i DynamicWidget (container).
  Eier tween-animasjon av verdi/ring, verdi-puls ved endring og long-press-menyen.
-->
<script lang="ts">
	...
	// Tween numerisk verdi og pct mellom dataoppdateringer. Første treff setter target
	// med duration:0 så vi unngår en "0 → verdi"-animasjon ved første render.
	...
	$effect(() => {
		...
		if (!tweenInitialized) {
			...
			if (prevCurrent !== null && d.current !== null && prevCurrent !== d.current) {
				valuePulse = true;
				setTimeout(() => (valuePulse = false), 600);
			}
