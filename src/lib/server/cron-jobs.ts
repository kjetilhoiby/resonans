/**
 * Registeret over alle cron-jobber — flyttet hit fra `/api/cron/jobs/+server.ts`
 * slik at den interne dispatcheren (`cron-dispatcher.ts`) og endepunktet leser
 * samme liste. Endepunktet re-eksponerer den over HTTP for GitHub Actions.
 *
 * Legg til nye cron-jobber her — ingen endringer nødvendig i workflow eller
 * dispatcher.
 */
...
export const CRON_JOBS: CronJob[] = [
	...
	},
	{
		...
		schedule: '0 * * * *',
		description:
			'Timebasert lokal nudge for planlegg dag (07:00), parsjekk morgen (08:30) og avslutt dag (21:00)'
	},
	{
		path: '/api/cron/project-followup-nudges',
		schedule: '0 * * * *',
		description:
			'Timebasert lokal purre-nudge for prosjekt-oppfølging (kontakter med forfalt oppfølgingsdato, default 09:00)'
	},
	{
		path: '/api/cron/theme-research',
		schedule: '20 4 * * *', // 04:20 UTC = 06:20 Oslo — forhåndshent research for kommende reiser
		description:
			'Forhåndshenter research for kommende reise-temaer (destinasjon + startdato i nær framtid, uten funn fra før)',
	...
	},
	{
		...
		description:
			'Ukentlig dagligvare-oppsummering (forrige uke vs. snitt/budsjett, plan-vs-kjøp)',
		maxDurationSeconds: 120
	},
	{
	...
	},
	{
		...
		description:
			'Ukentlig justering av adaptive treningsprogrammer — tempo, dagplassering og volum for neste uke',
		maxDurationSeconds: 120
	},
	{
		...
		description:
			'Kveldlig systemhelsesjekk — sensor-ferskhet, jobb-helse, cron-eksekvering → Google Chat',
	...
	// rekkefølgen dag → uke → måned → år holdes selv 1. januar.
	{
		...
		description:
			'Daglig drøm-syntese (alle brukere) — leser refleksjoner + plan-artefakter + egenfrekvens',
		maxDurationSeconds: 300
	},
	{
