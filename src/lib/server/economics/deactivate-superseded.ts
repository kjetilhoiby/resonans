...
import { diagnoseDuplicates, type DuplicateDiagnosis } from './duplicate-diagnosis';
...
export type DeactivateResult = {
	 ...
	 *
	 * **Samme beregning som `GET /api/admin/economics/duplikater`**, som er den man skal polle:
	 * den er ren lesing og tar tersklene som parametere. Feltet står her fordi svaret på «hvorfor
	 * står disse igjen» hører der planen leses.
	 */
	residual: DuplicateDiagnosis;
...
export async function deactivateSupersededReservations(
	...
	// **Radene leses med den DELTE lasteren**, som også betjener diagnoseendepunktet.
	// Statusnormaliseringen og overføringsvakten er kalibrering, ikke plumbing: to kopier ville
	// svart på litt ulike spørsmål uten at noe sa fra.
	const loaded = await loadDuplicateCandidates(userId, { days: options.days });
	const fromDate = loaded.fromDate;

	const candidates: ReservationCandidate[] = loaded.rows.map((row) => ({
		id: row.id,
	...
	// **Restposten måles på det som står IGJEN etter denne kjøringen**, ikke på alt. Ellers ville
	// hvert par ryddingen fanget også dukket opp som «skulle-blitt-fanget», og tallet hadde svart
	// på et annet spørsmål enn brukerens: «hvorfor står disse der ennå?».
	//
	// Samme beregning som diagnoseendepunktet, med standardtersklene.
	const residual = await diagnoseDuplicates(userId, {
		days: options.days,
		excludeIds: new Set(selected.map((m) => m.reservationId))
	});
	...
	return {
		...
		skippedUnknownStatus: loaded.counts.unknownStatus,
		skippedInternalTransfers: loaded.counts.pairedTransfers,
		skippedTransferText: loaded.counts.textOnlyTransfers,
		pairs: {
