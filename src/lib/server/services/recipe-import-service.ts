/**
 * recipe-import-service.ts — henter en nettside og trekker ut oppskriften
 * (JSON-LD Recipe prioriteres, ellers gpt-4o-mini over sideteksten), og
 * lagrer den i kartoteket (meals). Delt av import-endepunktet og
...
Hvis siden ikke inneholder en oppskrift: returner {"error": "ingen oppskrift funnet"}.`
	...
	if (parsed.error || !parsed.title) {
		return { ok: false, error: parsed.error ?? 'Fant ingen oppskrift på siden.', status: 422 };
	}
