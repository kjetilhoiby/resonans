...
describe('bevegelsestid vinner over elapsed', () => {
	/**
	 * Saken som utløste feltet: en el-sykkeltur der sporingen ble glemt sto som
	 * 2 t 20 min og fikk effort 114. MET-stien er rent lineær i varighet, så
	 * testene uttrykker FORHOLD — hardkodede nivåer låser seg til kalibreringen.
	 */
	const glemtSporing: WorkoutEffortInput = {
		sportType: 'e_bike',
		durationSeconds: 8400,
		movingSeconds: 1620
	};

	it('skårer på bevegelsestiden når den finnes', () => {
		const med = computeWorkoutEffort(glemtSporing, baseline);
		const uten = computeWorkoutEffort({ ...glemtSporing, movingSeconds: null }, baseline);

		expect(med?.durationBasis).toBe('moving');
		expect(uten?.durationBasis).toBe('elapsed');
		// Forholdet mellom skårene skal være forholdet mellom varighetene.
		expect(med!.score / uten!.score).toBeCloseTo(1620 / 8400, 3);
	});

	it('gir samme skår som en økt der hele opptaket var bevegelse', () => {
		const glemt = computeWorkoutEffort(glemtSporing, baseline);
		const ren = computeWorkoutEffort(
			{ sportType: 'e_bike', durationSeconds: 1620 },
			baseline
		);

		expect(glemt!.score).toBe(ren!.score);
	});

	it('faller tilbake på elapsed når sporet ikke kunne svare', () => {
		for (const moving of [null, undefined, 0]) {
			const result = computeWorkoutEffort({ ...glemtSporing, movingSeconds: moving }, baseline);
			expect(result?.durationBasis).toBe('elapsed');
		}
	});

	it('måler minstelengden på bevegelsestiden, ikke på opptaket', () => {
		// En time stillstand med to minutter sykling er ikke en økt. Målte vi
		// porten på elapsed, ville den sluppet gjennom og blitt skåret som to minutter.
		const result = computeWorkoutEffort(
			{ sportType: 'cycling', durationSeconds: 3600, movingSeconds: 120 },
			baseline
		);
	...
	it('gjelder også TRIMP-stien', () => {
		const med = computeWorkoutEffort(
			{ sportType: 'cycling', durationSeconds: 8400, movingSeconds: 1620, avgHeartRate: 140 },
			baseline
		);
		const uten = computeWorkoutEffort(
			{ sportType: 'cycling', durationSeconds: 8400, avgHeartRate: 140 },
			baseline
		);

		expect(med?.method).toBe('trimp');
		expect(med!.score / uten!.score).toBeCloseTo(1620 / 8400, 3);
	});
});
